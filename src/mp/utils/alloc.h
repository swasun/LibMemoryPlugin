...
#ifndef MEMORYPLUGIN_ALLOC_H
#define MEMORYPLUGIN_ALLOC_H
 ...
 */
#define mp_safe_alloc(var, type, size) \
	if (size <= 0) { \
...
#define mp_safe_alloc_ret(var, type, size, ret) \
	if (size <= 0) { \
		ei_stacktrace_push_msg("Can't allocate data with a negative or null size '%d'", (int)size); \
		ret = 0; \
	} else { \
 ...
 */
#define mp_safe_alloc_or_goto(var, type, size, label) \
	if (size <= 0) { \
		ei_stacktrace_push_msg("Can't allocate data with a negative or null size '%d'", (int)size); \
		goto label; \
	} \
	...
	var = (type*)malloc(size * sizeof(type)); \
	memset(var, 0, size * sizeof(type)); \
    mp_check_alloc_or_goto(var, label) \
 ...
 *  Check if variable is correctly allocated.
 */
#define mp_safe_realloc(var, type, old_size, more_size) \
	if (old_size < 0) { \
 ...
 *  Check if variable is correctly allocated. If not, go to specified label
 */
#define mp_safe_realloc_or_goto(var, type, old_size, more_size, label) \
	if (old_size < 0) { \
		ei_stacktrace_push_msg("Can't allocate data with a negative old_size '%d'", (int)old_size); \
	    goto label; \
	} \
 ...
 */
#define mp_check_alloc_or_goto(var, label) \
	if (errno == ENOMEM) { \
        ei_stacktrace_push_errno() \
        mp_safe_free(var) \
        goto label; \
 ...
 */
#define mp_safe_free(var) \
	if (var) { \
		free((void*)var); \
		var = NULL; \
	} \

#define mp_safe_str_free(str) \
	if (str) { \
		if (strcmp(str, "") != 0) { \
			free((void *)str); \
			str = NULL; \
